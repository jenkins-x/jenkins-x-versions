...
jx step bdd --test-git-repo=https://github.com/pmuir/bdd-jx.git --use-revision --versions-repo https://github.com/jenkins-x/jenkins-x-versions.git --config jx/bdd/ekstekton/cluster.yaml --base-domain=jxbdd.beescloud.com --gopath /tmp --git-provider=github --git-username $GH_USERNAME --git-owner $GH_OWNER --git-api-token $GH_CREDS_PSW --no-delete-app --no-delete-repo --tests install --tests test-create-spring